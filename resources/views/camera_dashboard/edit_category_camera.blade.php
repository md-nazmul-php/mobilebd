...
@section('title','Camera - Edit Category')
@section('content')
...
<div class="">
	<div class="col-md-12 ml-auto">
		<div class="text-center">
			<h2 class="text-success">Edit Camera Category</h2>
		</div>
		<div class="mt-4 mb-3 px-2">
			<div class="my-2">
				<a class="btn btn-success btn-sm py-0" href="{{route('camera.category')}}">Add New Category</a>
			</div>
			<form method="POST" action="{{route('edit_camera_category.save')}}">
				@csrf
				<div class="form-group">
					...
					<input type="text" name="id" value="{{$camera_cat_edit->id}}" hidden="hidden">
				    <label for="category_name">Edit Category Name :</label>
				    <span class="text-danger">@error ('category_name'){{$message}} @enderror</span>
				    <input type="text" class="form-control" name="category_name" id="category_name" value="{{$camera_cat_edit->category_name}}">
				</div>
				<div class="form-group">
					<label for="brand_name">Edit Description :</label>
				    <span class="text-danger">@error ('description'){{$message}} @enderror</span>
				    <textarea class="form-control" id="html_editor" name="description" rows="4">{{$camera_cat_edit->description}}</textarea>
				</div>
				<div class="mt-5">
