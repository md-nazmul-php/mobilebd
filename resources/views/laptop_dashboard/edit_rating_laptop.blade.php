...
@section('title','Laptop - Edit Rating')
@section('content')
<div class="mb-2 pb-1 border-bottom">
	...
	<a class="px-3 py-1 bg-secondary text-white" href="{{route('laptop.rating')}}">Rating</a>
...
<div class="">
	<div class="col-md-12 ml-auto">
		<div class="text-center">
			<h2 class="text-success">Edit Laptop Rating</h2>
		</div>
		<div class="mt-5 mb-3 px-2">
			<div class="my-2">
				<a class="btn btn-success btn-sm py-0" href="{{route('laptop.rating')}}">Add New Rating</a>
			</div>
			<form method="POST" action="{{route('edit_laptop_rating.save')}}">
				@csrf
				<div class="form-group">
					...
					<input type="text" name="id" value="{{$laptop_rating_edit->id}}" hidden="hidden">
				    <label for="rating_name">Edit Rating Name :</label>
				    <span class="text-danger">@error ('rating_name'){{$message}} @enderror</span>
				    <input type="text" class="form-control" name="rating_name" id="rating_name" value="{{$laptop_rating_edit->rating_name}}">
				    <input type="hidden" name="_token" value="{{ Session::token() }}">
				</div>
				<div class="form-group">
					<label for="brand_name">Edit Description :</label>
				    <span class="text-danger">@error ('description'){{$message}} @enderror</span>
				    <textarea class="form-control" id="html_editor" name="description" rows="4">{{$laptop_rating_edit->description}}</textarea>
				</div>
				<div class="mt-5">
