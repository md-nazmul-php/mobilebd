...
@section('title','Laptop - Edit Brands')

@section('content')
<div class="mb-2 pb-1 border-bottom">
	<a class="px-3 py-1" href="{{route('laptop.all')}}">Products</a>
	<a class="px-3 py-1 bg-secondary text-white" href="{{route('laptop.brand')}}">Brand</a>
...
<div class="">
	<div class="col-md-12 ml-auto">
		<div class="text-center">
			<h2 class="text-success">Edit Laptop Brand</h2>
		</div>
		<div class="mt-5 mb-3 px-2">
			<div class="my-2">
				<a class="btn btn-success btn-sm py-0" href="{{route('laptop.brand')}}">Add New Brand</a>
			</div>
			<form method="POST" action="{{route('edit_laptop_brand.save')}}" enctype="multipart/form-data">
				@csrf
				<div class="form-group">
					...
					<input type="text" name="id" value="{{$laptop_brand_edit->id}}" hidden="hidden">
				    <label for="brand_name">Edit Brand Name :</label>
				    <span class="text-danger">@error ('brand_name'){{$message}} @enderror</span>
				    <input type="text" class="form-control" name="brand_name" id="brand_name" value="{{$laptop_brand_edit->brand_name}}">
				</div>
				<div class="form-group">
					<label for="brand_name">Edit Description :</label>
				    <span class="text-danger">@error ('description'){{$message}} @enderror</span>
				    <textarea class="form-control" id="html_editor" name="description" rows="4">{{$laptop_brand_edit->description}}</textarea>
					
				</div>
				<div class="form-group">
					<label for="brand_img">Change Brand image :</label>
					<input class="border-0" type="file" name="brand_img" id="brand_img">
	            	<input type="text" name="imgName" value="{{$laptop_brand_edit->brand_img}}" hidden="hidden">
				</div>
				<div>
					@if($laptop_brand_edit->brand_img=='')
			          <img class="img" id="brand_img_priveiw" width="190" height="80" src="{{asset('admin_dashboard/images')}}/no_image.jpg">    
			          @else
			          <img class="img" id="brand_img_priveiw" width="190" height="80" src="{{asset('admin_dashboard/brand_img')}}/{{$laptop_brand_edit->brand_img}}">
			        @endif
				</div>
